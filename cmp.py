...
class CommandInterpreter(cmd.Cmd):
    ...
    def precmd(self, line):
        """Executes before interpreting the line."""
        if not line:
            return '\n'
    ...
    def do_EOF_command(self, line):
        """EOF command to gracefully exit."""
        print("")
        return True

    def do_quit_command(self, arg):
        """Quits the program."""
        return True

    def empty_line(self):
        """Override default empty line behavior."""
        pass

    def do_create_command(self, arg):
    ...
    def do_destroy_command(self, arg):
        """Deletes an instance."""
        args = arg.split()
        if not validate_classname(args, check_id=True):
            return
    ...
    def do_update_command(self, arg: str):
        """Updates an instance."""
...
def validate_attrs(args):
    """Validates classname attributes and values."""
    if len(args) < 3:
...
def parse_str(arg):
    """Parses `arg` to an `int`, `float` or `string`."""
    parsed = re.sub("\"", "", arg)
